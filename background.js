...
Their submitted solution (${normalizedLanguage}):
\`\`\`${normalizedLanguage}
${userCode}
\`\`\`
...
## 📊 Complexity of Submitted Solution
**Time:** O(?) — [brief reason]
...
The student's solution (${normalizedLanguage}):
\`\`\`${normalizedLanguage}
${userCode}
\`\`\`

Write the most optimal solution possible in ${normalizedLanguage}.
...
4. Include both complexity lines near the top of the code as comments, exactly in this text form:
  Time: O(...)
  Space: O(...)
...
The code must be runnable and complete.`;
    ...
    sendResponse({
      ...
      optimizedCode: cleanCode(optimizedResponse.content, normalizedLanguage),
    });
  } catch (err) {
...
async function handlePushToGithub(data, sendResponse) {
  try {
    ...
    const solutionContent = buildComprehensiveSolution({
      title, difficulty, url, timestamp, number, language: normalizedLanguage,
...
function buildComprehensiveSolution({ title, difficulty, url, timestamp, number, language, description, userCode, optimizedCode, approach }) {
  const sections = [];
  const submittedComplexity = extractComplexityFromApproach(approach, "submitted");
  const optimizedComplexityFromApproach = extractComplexityFromApproach(approach, "optimized");
  const optimizedComplexityFromCode = extractComplexityFromCode(optimizedCode);
  const optimizedComplexity = mergeComplexity(optimizedComplexityFromApproach, optimizedComplexityFromCode);
...
**Complexity Analysis:**
- Time Complexity: ${submittedComplexity.time || "Not provided"}
- Space Complexity: ${submittedComplexity.space || "Not provided"}
...
`);
  ...
  > **Generated by:** openai/gpt-oss-120b on ${timestamp}
...
- Time Complexity: ${optimizedComplexity.time || "Not provided"}
- Space Complexity: ${optimizedComplexity.space || "Not provided"}
...
function normalizeLanguage(raw) {
  ...
  if (/^\d+\.?$/.test(lang)) return "text";
  return lang;
}

function extractComplexityFromApproach(approach, target) {
  const fallback = { time: null, space: null };
  ...
  if (target === "submitted") {
    const idx = lower.indexOf("complexity of submitted solution");
    if (idx >= 0) section = approach.slice(idx, idx + 1200);
  }

  if (target === "optimized") {
    const idx = lower.indexOf("complexity of optimized solution");
    if (idx >= 0) section = approach.slice(idx, idx + 1200);
  }

  const source = section || approach;
  return {
    time: extractComplexityLine(source, "time"),
    space: extractComplexityLine(source, "space"),
  };
}

function extractComplexityFromCode(code) {
  if (!code) return { time: null, space: null };
  return {
    time: extractComplexityLine(code, "time"),
    space: extractComplexityLine(code, "space"),
  };
}

function mergeComplexity(primary, secondary) {
  return {
    time: primary.time || secondary.time || null,
    space: primary.space || secondary.space || null,
  };
}

function extractComplexityLine(text, label) {
  if (!text) return null;
  ...
  for (const line of lines) {
    ...
    // Normalize markdown/comment noise so formats like **Time:** O(n) parse correctly.
    const cleaned = line
      .replace(/[`*_>#-]/g, " ")
      .replace(/\/\//g, " ")
      .replace(/\s+/g, " ")
    ...
    const explicit = cleaned.match(new RegExp(`${label}\\s*(complexity)?\\s*[:=-]?\\s*(.+)$`, "i"));
    if (explicit && explicit[2]) {
      const value = sanitizeComplexityValue(explicit[2]);
      if (value) return value;
    }

    const bigOOnly = cleaned.match(/((?:O|Θ|Omega|Ω)\s*\([^\)\n\r]{1,60}\))/i);
    if (bigOOnly && bigOOnly[1]) {
      return sanitizeComplexityValue(bigOOnly[1]);
    }
  }

  // Fallback: search near the keyword in the whole text block.
  const nearby = text.match(new RegExp(`${label}[\\s\\S]{0,180}?((?:O|Θ|Omega|Ω)\\s*\\([^\\)\\n\\r]{1,60}\\))`, "i"));
  if (nearby && nearby[1]) {
    return sanitizeComplexityValue(nearby[1]);
  }

  return null;
}

function sanitizeComplexityValue(raw) {
  if (!raw) return null;
  return raw
    .replace(/^[-:;\s]+/, "")
    .replace(/[\s.,;:]+$/, "")
